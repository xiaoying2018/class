    ...
    methods = {
        ...
        readySendData: function() {
            if (!$(formDv.vars.submitBtn).hasClass("disabled")) {
				...
				if ($("#interCode").text() && $("#interCode").text().length > 0 && $("#interCode").text().indexOf('+86') == -1) {
                    _this._valid.XY_a02 = 'required'
                }
                ...
                if (!result) {
					...
					// _this._sendData['XY_b35'] = $("#interCode").text().split('+')[1];
