...
class MaterialsModel extends CRMBaseModel
{
    ...
    {
        return $this->db(1,"DB_CONFIG_EDU")->execute('insert into `mx_materials` (`name`,`cate_id`,`program_id`,`student_id`,`create_time`,`file`,`sname`,`program_name`) values ("'.$data['name'].'",'.$data['cate_id'].','.$data['program_id'].','.$data['student_id'].','.$data['create_time'].',"'.$data['file'].'","'.$data['sname'].'","'.$data['project_name'].'")');
    }
